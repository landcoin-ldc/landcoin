...
Value getsubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetPOSReward(nBestHeight, 0);
}
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("blockvalue",    (uint64_t)GetPOSReward(nBestHeight, 0)));
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "GreenCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "GreenCoin is downloading blocks...");
